  ...
  return rhi::ResourceTransferRHI{_taskExecutor.enqueue(
      ...
      [this, id, uploadShader = std::move(uploadShader)]() {
        ...
        for (std::size_t i = 0; i < rhi::ShaderPermutationRHI::count; ++i) {
          if (uploadShader.code[i].empty()) {
            continue;
          }

          shaderModuleCreateInfo.codeSize =
              uploadShader.code[i].size() * sizeof(uploadShader.code[i][0]);
          shaderModuleCreateInfo.pCode = reinterpret_cast<std::uint32_t const*>(
              uploadShader.code[i].data());

          if (vkCreateShaderModule(_vkDevice, &shaderModuleCreateInfo, nullptr,
                                   &shader.permutations[i])) {
    ...
    for (std::size_t i = 0; i < rhi::ShaderPermutationRHI::count; ++i) {
      ...
      pipelineBuilder._vertexInputDescription =
          getVertexInputDescriptionForPermutation(
              static_cast<rhi::ShaderPermutationRHI::Type>(i),
              uploadMaterial.materialType == core::MaterialType::pbr);
      ...
      pipelineBuilder._vkShaderStageCreateInfos.push_back(
          vkinit::pipelineShaderStageCreateInfo(
              VK_SHADER_STAGE_VERTEX_BIT, vertexShaderModule.permutations[i]));

      pipelineBuilder._vkShaderStageCreateInfos.push_back(
          vkinit::pipelineShaderStageCreateInfo(
              VK_SHADER_STAGE_FRAGMENT_BIT,
              fragmentShaderModule.permutations[i]));

      newMaterial.vkPipelineMainRenderPass[i] =
          pipelineBuilder.buildPipeline(_vkDevice, _mainRenderPass);

      setDbgResourceName(_vkDevice,
                         (std::uint64_t)newMaterial.vkPipelineMainRenderPass[i],
      ...
      newMaterial.vkPipelineEnvironmentRendering[i] =
          pipelineBuilder.buildPipeline(_vkDevice, _envMapRenderPass);
      pipelineBuilder._vkRasterizationCreateInfo.frontFace =
          VK_FRONT_FACE_COUNTER_CLOCKWISE;

      setDbgResourceName(
          _vkDevice,
          (std::uint64_t)newMaterial.vkPipelineEnvironmentRendering[i],
          VK_OBJECT_TYPE_PIPELINE, uploadMaterial.debugName,
    ...
    if (uploadMaterial.materialType == core::MaterialType::lit) {
      ...
      if (hasDiffuseTex) {
        ...
        descriptorBuilder.bindImage(
            1, diffuseTexImageInfo, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
      ...
      if (hasNormalMap) {
        ...
        descriptorBuilder.bindImage(
            2, normalMapTexImageInfo, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
            VK_SHADER_STAGE_FRAGMENT_BIT, nullptr, true);
      } else {
        descriptorBuilder.declareUnusedImage(
...
void VulkanRHI::destroyUnusedResources(bool forceDestroy) {
  ...
  for (PendingResourcesToDestroy::ResourceEntry const& shaderEntry :
    ...
    if (forceDestroy || shaderEntry.lastUsedFrame <= lastRenderedFrame) {
      ...
      for (std::size_t i = 0; i < shader.permutations.size(); ++i) {
        if (shader.permutations[i] != VK_NULL_HANDLE) {
          vkDestroyShaderModule(_vkDevice, shader.permutations[i], nullptr);
        }
      }
